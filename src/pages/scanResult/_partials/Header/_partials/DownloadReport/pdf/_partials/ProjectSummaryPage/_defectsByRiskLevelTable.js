...
const DefectsByRiskLevelTable = props => {
	...
	const headRows = [
		...
		{
			field: 'rule-standard',
			label: i18n.t('pages.scan-result.pdf.table.rule-standard'),
			className: 'table-header',
		},
		{
			field: 'percentage',
			...
			Object.keys(summaryByCsvCode).map((csvCode, index) => {
				...
				return (
					<tr key={index}>
						...
						<td>
							<span>{ruleInfo.code || csvCode}</span>
						</td>
						<td>{ruleInfo.name || ''}</td>
						<td>
							<span>
								{ruleInfo.ruleSet && ruleInfo.ruleSet.displayName}
							</span>
						</td>
						<td>
	...
	return (
		<div className={`${locale} defects-summary-table-pdf`}>
			<div className='table-title'>
				{i18n.t('pages.scan-result.pdf.project-summary.defects-by-risk-level')}
			</div>
